...
def lambda_handler(event, context):
    ...
    for element in covid_array:
        objCount = get_s3_object_count('projectpro-covid19-test-data-akash7','covid19/'+element+'/time_series_covid19_'+element+'_global.csv')
        if objCount != 0:
            file_response = s3.get_object(Bucket='projectpro-covid19-test-data-akash7', Key='covid19/'+element+'/time_series_covid19_'+element+'_global.csv')
            
            file_content = file_response['Body'].read().decode('utf-8')
            
            s3.put_object(Bucket='projectpro-covid19-test-data-akash7', Key='covid19/archival/'+element+'/'+date+'/time_series_covid19_'+element+'_global.csv', Body=file_content)
        ...
        with urllib.request.urlopen(url) as response:
            data = response.read()
            s3.put_object(Bucket='projectpro-covid19-test-data-akash7', Key='covid19/'+element+'/time_series_covid19_'+element+'_global.csv', Body=data)
